...
func run() -> Dictionary:
	var failures: Array = []
	var director: RunDirector = RunDirector.new()
	...
	var run_rooms: Array = director.build_run(123456, room_catalog, 3)
	...
	for room_data: Variant in run_rooms:
		if not RunDirector.validate_room(room_data as Dictionary):
