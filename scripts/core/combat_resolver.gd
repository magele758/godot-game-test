...
static func compute_hit(base_damage: float, attacker_bonus: float, combo_depth: int, is_execution: bool = false) -> Dictionary:
	var combo_multiplier := 1.0 + min(0.35, float(max(0, combo_depth - 1)) * 0.05)
	var execution_multiplier := 1.8 if is_execution else 1.0
	var damage := int(round(base_damage * attacker_bonus * combo_multiplier * execution_multiplier))
	return {
		"damage": max(1, damage),
